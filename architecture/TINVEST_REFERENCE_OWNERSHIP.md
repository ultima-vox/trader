...
`TradingSchedules` compares `from` with current UTC provider date before dispatch. Historical ranges return typed `HistoricalRangeUnsupported`; they are never clamped. Valid future ranges preserve caller intent by splitting into closed provider-legal windows of at most 14 days. Responses merge in request/provider order, identical boundary rows deduplicate, conflicting rows fail closed, and every chunk retains response metadata. Any failed chunk returns an explicit partial-progress error; no incomplete schedule is exposed as complete.
